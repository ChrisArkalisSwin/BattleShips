...
{
	public static int FIELD_TOP;
	public static int FIELD_LEFT;
	public static int FIELD_WIDTH;

	public static int FIELD_HEIGHT;

	public static int MESSAGE_TOP;
	public static int CELL_WIDTH;
	public static int CELL_HEIGHT;

	public static int CELL_GAP;
    public static int CELL_GAP_DEPLOY;

    public static int CELL_HEIGHT_DEPLOY;

    public static int SHIP_GAP;
	...
	int cellGap)
	{
		...
		//Draw the grid
		for (int row = 0; row <= GameController._TileSetting -1; row++) {
			rowTop = top + (cellGap + cellHeight) * row;

			for (int col = 0; col <= GameController._TileSetting - 1; col++) {
		...
		foreach (Ship s in thePlayer) {
			if (s == null || !s.IsDeployed)
				continue;
			rowTop = top - 1 + (cellGap + cellHeight) * s.Row + SHIP_GAP;
			colLeft = left - 2 + (cellGap + cellWidth) * s.Column + SHIP_GAP;
