...
public class SeaGrid : ISeaGrid
{
	...
	public int _WIDTH = GameController._TileSetting;

	public int _HEIGHT = GameController._TileSetting;
	...
	public AttackResult HitTile(int row , int col )
	{
		try {
			...
			if (_GameTiles[row, col].Ship.IsDestroyed) {
				...
				_ShipsKilled += 1;
				return new AttackResult(ResultOfAttack.Destroyed, _GameTiles[row, col].Ship, "Sunk the enemy's", row, col);
			}

			//else hit but not destroyed
			return new AttackResult(ResultOfAttack.Hit, "hit an enermy vessel!", row, col);
		} finally {
			if (Changed != null) {
				Changed(this, EventArgs.Empty);
			}
